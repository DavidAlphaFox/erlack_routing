...
parse_rules(Dispatches) ->
    Dispatches1 =
        [{Name, [{L, erlack_url_rule:parse(R), A} || {L,R,A} <- D ]}
         || {Name, D} <- Dispatches ],
...
parse_regex(Patterns) ->
    Patterns1 =
        [ {Name,{L,erlack_regex:parse(R),D,E}}
...
build_url_dispatch(Dispatches, Patterns) ->
    ...
    of
        [] ->
            {Tags, EndStates, DFA} = erlack_tagged_nfa:to_tagged_dfa(NFA, TaggedTrans, maps:keys(Endpoints)),
            {ok,
               ...
               {map, 1,
                [ {map_field_assoc, 1, {atom,1, K},
                   {call,1,{remote,1,{atom,1,M},{atom,1,F}},
                    [{call,1,{remote,1,{atom,1,erlack_url},{atom,1,decode}},
              ...
              {Args3, CurrentState3, MaxState3, NFA3, Tags3, TaggedTrans3} =
                  lists:foldl(
                    fun({N, Trans}, {Args2, State, MaxState2, NFA2, Tags2, TaggedTrans2}) when is_integer(N) ->
                            ...
                            {Args2, EndState, EndState, erlack_nfa:renum(Trans, MaxState2 - 1, State, N, EndState) ++ NFA2, Tags2, TaggedTrans2};
                       ({Name2, Pattern}, {Args2, State, MaxState2, NFA2, Tags2, TaggedTrans2}) ->
                            ...
                            { Args2#{Name2 => {Tags2, Tags2+1, D}},
                              EndState+1,
                              EndState+1,
                              erlack_nfa:renum(Trans, MaxState2, N) ++ NFA2,
                              Tags2+2,
...
re2nfa(RE) ->
    RE1 = erlack_url_regex:encode(erlack_utf8_regex:encode(RE)),
    {End, NFA} = erlack_nfa:from_re(RE1),
    {States, DFA} = erlack_nfa:to_dfa(NFA),
    ...
    {States1, DFA1} = erlack_dfa:minimize(DFA, [Group1,Group2]),
    Size = length(DFA1),
    Group2 =
        lists:usort(
...
format_segment({Name, Type}, Patterns) ->
    ...
    {call,1,
     {remote,1,{atom,1, erlack_url},{atom,1,encode}},
